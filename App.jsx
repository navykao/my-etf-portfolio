        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          <div className="lg:col-span-5 space-y-5">
            ...
            <section className="bg-white p-5 rounded-2xl shadow-sm border border-stone-200/60">
              ...
              <div className="space-y-3">
                <div><label className="text-[11px] font-medium text-stone-600">เงินลงทุนเริ่มต้น (บาท)</label><input type="number" value={initialInvestment ?? ''} onBlur={e => { if (e.target.value === '') handleUpdateSetting(setInitialInvestment, 'initialInvestment', 0); }} onChange={e => handleUpdateSetting(setInitialInvestment, 'initialInvestment', e.target.value === '' ? 0 : Number(e.target.value))} className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-2.5 font-bold text-sm outline-none focus:border-teal-300 focus:bg-white mt-1 transition-all" /></div>
                <div><label className="text-[11px] font-medium text-stone-600">ลงทุนเพิ่มรายเดือน (บาท)</label><input type="number" value={monthlyContribution ?? ''} onBlur={e => { if (e.target.value === '') handleUpdateSetting(setMonthlyContribution, 'monthlyContribution', 0); }} onChange={e => handleUpdateSetting(setMonthlyContribution, 'monthlyContribution', e.target.value === '' ? 0 : Number(e.target.value))} className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-2.5 font-bold text-sm outline-none focus:border-teal-300 focus:bg-white mt-1 transition-all" /></div>
                <div className="grid grid-cols-2 gap-3">
                  <div><label className="text-[11px] font-medium text-stone-600">เพิ่มปีละ (%)</label><input type="number" value={contributionStepUp ?? ''} onBlur={e => { if (e.target.value === '') handleUpdateSetting(setContributionStepUp, 'contributionStepUp', 0); }} onChange={e => handleUpdateSetting(setContributionStepUp, 'contributionStepUp', e.target.value === '' ? 0 : Number(e.target.value))} className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-2.5 font-bold text-sm outline-none focus:border-teal-300 focus:bg-white mt-1 transition-all" /></div>
                  <div><label className="text-[11px] font-medium text-stone-600">ระยะเวลา (ปี)</label><input type="number" value={investmentYears ?? ''} onBlur={e => { if (e.target.value === '' || Number(e.target.value) < 1) handleUpdateSetting(setInvestmentYears, 'investmentYears', 1); }} onChange={e => handleUpdateSetting(setInvestmentYears, 'investmentYears', e.target.value === '' ? 1 : Math.max(1, Number(e.target.value)))} className="w-full bg-stone-50 border border-stone-100 rounded-xl px-4 py-2.5 font-bold text-sm outline-none focus:border-teal-300 focus:bg-white mt-1 transition-all" /></div>
                </div>
              </div>
            </section>
          </div>
