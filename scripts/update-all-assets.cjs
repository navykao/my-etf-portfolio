...
const CONFIG = {
  ...
  YAHOO_BATCH_DELAY_MS: 3000,  // 3 วินาที ระหว่าง batch
  EODHD_DELAY_MS: 3500,
...
// ============================================
// YAHOO HEADERS — แก้ปัญหา HTTP 401 จาก GitHub Actions
// ============================================
const YAHOO_HEADERS = {
...
// HELPERS
// ============================================
// ✅ แก้ไข: รับ options เพิ่มเติมได้ (สำหรับ headers)
async function fetchWithTimeout(url, timeoutMs = CONFIG.FETCH_TIMEOUT_MS, options = {}) {
...
//
// ✅ แก้ไข:
//   - เปลี่ยนจาก query1 → query2 (หลวมกว่า)
//   - เปลี่ยนจาก /v7/ → /v8/ (endpoint ใหม่)
//   - เพิ่ม Browser Headers เพื่อหลีกเลี่ยง HTTP 401
// ============================================
...
async function updateAllAssets() {
  ...
  console.log(`  Yahoo:    ✅ No key needed (batch 50, query2/v8)\n`);
  ...
  //              marketCap, volume, avgVolume, high52w, low52w
  // ทำกับ ALL ตัว (ไม่ใช่แค่ที่ Finnhub fail)
  // ✅ แก้ไข: ใช้ query2/v8 + Browser Headers
  // ============================================
  console.log('\n' + '═'.repeat(55));
  ...
  console.log(`   Estimated time: ~${Math.ceil(existingData.length / CONFIG.YAHOO_BATCH_SIZE) * 3}s`);
  console.log(`   Endpoint: query2.finance.yahoo.com/v8 + Browser Headers`);
  console.log('═'.repeat(55));
